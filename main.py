...
rocket_controller = controller.RocketController()

# -----INITIALIZE PERIPHERALS-----
# Initialize IMU
imu = intializations.RocketIMU()
# Initialize SD Card
sd = intializations.RocketSDCard()
# Initialize UART
uart = intializations.RocketRadio()
# Initialize Pressure Sensor
pressure_sensor = intializations.RocketPressureSensor()
...
data_updater = controller.make_data_updater(queue_frequency, check_interval, (GRAVITY, ALTITUDE_THRESHOLD), (controller.time_queue, controller.accel_queue, controller.altitude_queue))

def find_reference_gravity():  # CHECK HERE FOR SOMEWHAT ARBITRARY VALUES
    global GRAVITY
    if rocket_timer.time_since_ms(controller.time_queue.peek()) < check_interval * 0.5:
        return
    mean = controller.accel_queue.getMean()
    ...
    if (9.5 < mean < 10.1):
        ...
        variance = controller.accel_queue.getVariance()
        if variance < 0.1:
...
controller.do_every([data_updater, find_reference_gravity],[accel_sample_interval, check_interval])
...
def find_reference_altitude():
    ...
    if rocket_timer.time_since_ms(controller.time_queue.peek()) < check_interval * 0.5:
        return
    mean = controller.altitude_queue.getMean()
    variance = controller.altitude_queue.getVariance()
    if variance < 10:
...
controller.do_every([data_updater, find_reference_altitude],[accel_sample_interval, check_interval])
...
data_updater = controller.make_data_updater(queue_frequency, burn_time, (GRAVITY, ALTITUDE_THRESHOLD), (controller.time_queue, controller.accel_queue, controller.altitude_queue))

def check_launch():
    if rocket_timer.time_since_ms(controller.time_queue.peek()) < 0.5 * burn_time:
        return  # Dont check for launch if there's not enough data in the queue yet
    uart.write("\n Launch Threshold Proportion: " + str(controller.accel_queue.get_proportion_above_threshold()))
    if controller.accel_queue.get_proportion_above_threshold() < 0.95:
        return  # ARBITRARY: If less than 95% of the data is above the gravity threshold, then the rocket has not launched
    if controller.altitude_queue.get_proportion_above_threshold() > 0.95:
...
controller.do_every([sd.write_to_imu_data, data_updater, check_launch], [imu_data_interval, accel_sample_interval, check_launch_interval])
data_updater = None
...
def check_landing():
    if rocket_timer.time_since_ms(controller.time_queue.peek()) < 0.5 * check_interval:
        return  # Dont check for landing if there's not enough data in the queue yet
    uart.write("\n Landing Threshold Proportion: " + str(controller.accel_queue.get_proportion_above_threshold()))
    if controller.accel_queue.get_proportion_above_threshold() > 0.05:
        return
    # ARBITRARY: If more than 5% of the data is above the gravity threshold, then the rocket has not landed
    if controller.controller.altitude_queue.get_proportion_above_threshold() < 0.05:
        raise StopIteration
...
controller.do_every([sd.write_to_imu_data, data_updater, check_landing], [imu_data_interval, accel_sample_interval, check_landing_interval])
